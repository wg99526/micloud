...
tax.trans2 <- function(otu.tab, tax.tab, rare.otu.tab, rare.tax.tab, sub.com = TRUE, na.code = "NANANA") {
...
#   return(taxon.names.rank)
# }

# taxa.names <- function(taxa.out){ # input should be taxa.out$count, taxa.out$clr or so (use taxa.names.rank but keep this just in case)
#   taxon.names <- list()
#   taxon.names <- lapply(taxa.out, function(x) str_split(names(x), ";"))
#   
#   taxon.names.rank <- list()
...
#   
#   
#   if (length(unlist(taxon.names.rank)) != length(unique(unlist(taxon.names.rank)))) {
#     # then there exists duplicated names
#     duplicated.taxons = unique(unlist(taxon.names.rank)[duplicated(unlist(taxon.names.rank))])
#     
#     for(i in 1:length(duplicated.taxons)){
#       duplicated.taxon <- duplicated.taxons[i]
#       print(duplicated.taxon)
#       ind.dup.list = lapply(taxon.names.rank, function(x) which(x %in% duplicated.taxon))
#       
#       #taxon.names.rank.revised <- taxon.names.rank
#       #duplicated.taxon1 <- duplicated.taxon
#       for(j in 1:6){
#         if(length(ind.dup.list[[j]]) != 0){
#           duplicated.taxon <- paste(duplicated.taxon,"*",collapse = "")
#           taxon.names.rank[[j]][ind.dup.list[[j]]] <- duplicated.taxon
#         }
#       }
#     }
#   }
...
taxa.bin.glm.nb.func <- function(bin.var, taxa) {
  ...
  for (i in 1:n.tax) {
    ...
    fit.logit <- try(glm.nb(f, data = dat), silent = TRUE)
    
    est <- tryCatch(summary(fit.logit)$coefficients[2,1], error = function(err) NA)
    std.err <- tryCatch(summary(fit.logit)$coefficients[2,2], error = function(err) NA)
    DF <- NA
    ci <- c(est - qnorm(0.975)*std.err, est + qnorm(0.975)*std.err)
    p.val <- tryCatch(summary(fit.logit)$coefficients[2,4], error = function(err) NA)
    out.logit <- c(est, std.err, DF, ci, p.val)
...
taxa.bin.logit.func <- function(bin.var, taxa){
  ...
  for (i in 1:n.tax) {
    taxon <- taxa[,i]
    fit.log <- glm(unlist(bin.var) ~ taxon, family = binomial())
  ...
  for(i in 1:6) {
    if(!is.null(taxa.out[[i]])){
      ...
      for (j in 1:n.tax) {
        taxon <- taxa[,j]
        if(scale == TRUE){
          fit.logit <- glm(bin.var ~ scale(taxon), family = "binomial")
        } else {
          fit.logit <- glm(bin.var ~ taxon, family = "binomial")
        }
...
all.taxa.bin.beta <- function(sel.bin.var, sel.ref, sel.com, sam.dat, taxa, multi.method = "BH"){
  
  tax.out <- list()
  for (i in 1:6) {
    prop.taxa.out <- taxa.bin.cat.ref.beta.func(sel.bin.var = sel.bin.var, sel.ref = sel.ref, sel.com = sel.com, sam.dat = sam.dat, taxa[[i]])
    prop.taxa.test.out <- taxa.bin.beta.func(prop.taxa.out$bin.var, prop.taxa.out$taxa)
  ...
  for(i in 1:6) {
    if(!is.null(taxa.out[[i]])){
      ...
      for (j in 1:n.tax) {
        ...
        df <- summary(fit.logit)$df[2]
        std.err <- summary(fit.logit)$coefficients[2,2]
        #or.std.err <- exp(summary(fit.logit)$coefficients[2,2])
        or.se <- sqrt(exp(est)^2*diag(vcov(fit.logit)))[2]
        ci <- c(exp(est - qt(0.975, df)*std.err), exp(est + qt(0.975, df)*std.err))
        
        p.val <- summary(fit.logit)$coefficients[2,4]
        #out.logit <- c(exp(summary(fit.logit)$coefficients[2,1]), get.or.se2(fit.logit)[2], summary(fit.logit)$df[2], exp(confint(fit.logit)[2,]), summary(fit.logit)$coefficients[2,4])
        out.logit <- c(exp(est), or.se, df, ci, p.val)
        logit.out[j,] <- out.logit
      }
      logit.out <- as.data.frame(logit.out)
      rownames(logit.out) <- colnames(taxa)
...
taxa.bin.cov.glm.nb.func <- function(bin.var, cov.var, taxa) {
  if(!is.null(taxa)){
    ...
    for (i in 1:n.tax) {
      ...
      fit.logit <- try(glm.nb(f, data = dat, maxit = 50), silent = TRUE)
      est <- tryCatch(summary(fit.logit)$coefficients[2,1], error = function(err) NA)
      std.err <- tryCatch(summary(fit.logit)$coefficients[2,2], error = function(err) NA)
      df <- NA
      ci <- c(est - qnorm(0.975)*std.err, est + qnorm(0.975)*std.err)
