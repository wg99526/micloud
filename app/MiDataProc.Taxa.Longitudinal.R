...
taxa.bin.lmer.func <- function(bin.var, id.var, taxa) {
  if(!is.null(taxa)){
    ...
    for (i in 1:n.tax) {
      ...
      #out <- c(summary(fit)$coefficients[2,c(1,2,3)], confint(fit)[4,], summary(fit)$coefficients[2,5])
...
taxa.bin.logit.glmm.b.func <- function(bin.var, id.var, taxa, rare.count = FALSE) {
  if(!is.null(taxa)){
    ...
    for (i in 1:n.tax) {
      ...
      ci <- tryCatch(est + qnorm(c(0.025, 0.975))*std.err, error = function(err) c(NA,NA))
      df <- NA
...
#     
#     if(any(taxon == 0)) {
#       fit.zinb <- glmm.zinb(y ~ x + offset(log(library.size)), data = dat, random = ~1|cluster) # class(fit.zinb) == "zinbmm" "lme"
...
#     } else {
#       fit.nb <- glmm.nb(y ~ x + offset(log(library.size)), random = ~1|cluster)
...
#       df = NA
#       est <- summary(fit.nb)$coefficients$fixed[2]
#       std.err <- info$dist[2,"Std.Error"]
#       p.val <- info$dist[2,"pvalue"]
  ...
  if(!is.null(taxa)){
    ...
    for (i in 1:n.tax) {
      ...
      if(any(taxon == 0)) {
        ...
        fit.nb <- glmm.nb(y ~ x, random = ~1|cluster)
        #print(intervals(fit.nb)$fixed)
        info <- fixed.nb(fit.nb)
        ci.lower <- intervals(fit.nb, which = "fixed")$fixed[2,"lower"]
        ci.upper <- intervals(fit.nb, which = "fixed")$fixed[2,"upper"]
...
taxa.bin.cov.glmm.nb.func <- function(bin.var, id.var, cov.var, taxa) {
  if(!is.null(taxa)){
    ...
    for (i in 1:n.tax) {
      ...
      # if(any(taxon == 0)) {
      #   fit.zinb <- glmm.zinb(f, data = dat, random = ~1|cluster) # class(fit.zinb) == "zinbmm" "lme"
      #   info <- fixed.zi(fit.zinb)
      #   ci.lower <- intervals(fit.zinb, which = "fixed")$fixed[2,"lower"]
      #   ci.upper <- intervals(fit.zinb, which = "fixed")$fixed[2,"upper"]
      #   df = NA
